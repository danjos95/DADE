...
def main():
    ...
    n_total = 0
    delay_counter = [0]*300
    preproc_counter = [0]*300
    inference_counter = [0]*300
    preproc_calc = 0
    inference_calc = 0
    ...
    for sid, seq in enumerate(tqdm(seqs)):
        ...
        target_frame = 1
        remainder_time = 0.018
        prev_inf_time = 0.03
        feature_warmup_cnt = 0
        ...
        while 1:
            ...
            if opts.dynamic_schedule:
                ...
                if stride_cnt % opts.det_stride == 0:
                    ...
                    continue
            frame = frames[fidx]
            #Try to use when give artificial delay
            #time.sleep(0.03)
            #get current frame
            ...
            with torch.no_grad():
                ...
                t3 = perf_counter()
                target_frame, remainder_time = divmod(prev_inf_time+(t3-t1), 0.033)
                #if remainder_time > 0.017:
                target_frame=target_frame+1
                #inference based on target_frame
                #if target_frame is 0: buffer None = copy current feature
                #print(target_frame)
                if feature_warmup_cnt < 5:
                    result, buffer = model(frame, buffer=buffer, mode='on_pipe')
                    feature_warmup_cnt=feature_warmup_cnt+1
                elif int(target_frame)==0:    
                    result, buffer = model(frame, buffer=None, mode='on_pipe')
                elif int(target_frame)<6:
                    result, buffer = model(frame, buffer=buffer_queue[5-int(target_frame)], mode='on_pipe')
                else:
                    result, buffer = model(frame, buffer=buffer_queue[0], mode='on_pipe')
                #append buffer until len: 5
            ...
            prev_inf_time = t2-t3
            preproc_calc = preproc_calc+(t3-t1)
            inference_calc = inference_calc+(t2-t1)

            delay_counter[int(1000*(t2-t1))] = delay_counter[int(1000*(t2-t1))]+1
            preproc_counter[int(1000*(t3-t1))] = preproc_counter[int(1000*(t3-t1))] + 1
            inference_counter[int(1000*(t2-t3))] = inference_counter[int(1000*(t2-t3))] +1

        print(delay_counter)
        print(preproc_counter)
        print(inference_counter)
        print("calcs: ")
        print(preproc_calc)
        print(inference_calc)
